...
export function couponTypeRepositoryFactory({ db }: { db: Db }): CouponTypeRepository {
  async function findById(id: string): Promise<Maybe<CouponType>> {
    const rows = await db.select().from(couponTypes).where(eq(couponTypes.id, id)).limit(1);
    return rows[0] ?? null;
  }

  async function findByKey(key: string): Promise<Maybe<CouponType>> {
  ...
  async function upsertByKey(newCouponType: NewCouponType): Promise<void> {
    await db
      .insert(couponTypes)
  ...
  return { findById, findByKey, listAll, upsertByKey };
}
